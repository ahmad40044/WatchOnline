...
const navChange= () => setNavHidden(!navHidden) ;
const [darkMode, setDarkMode] = useState(
  JSON.parse(localStorage.getItem("darkMode")) || false
);
useEffect(() => {
  // Corrected localStorage key from "darkmode" to "darkMode"
...
<nav className="bg-white border-b-2 border-gray-200 dark:bg-gray-900 dark:border-b-2 dark:border-green-900">
  <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
  <Link to="/" className="flex items-center">
      <img src={logo} className="h-8 mr-3" alt="Watch-Online Logo" />
      <span className="self-center text- 2xl font-semibold whitespace-nowrap dark:text-white">WatchOnline</span>
  </Link>
  <div id="mobile-nav" className="flex md:order-2">
    ...
    <button type="button" onClick={navChange} data-collapse-toggle="navbar-search" aria-controls="navbar-search" aria-expanded="false" className="md:hidden text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5 mr-1" >
    ...
    <button  data-collapse-toggle="navbar-search" onClick={navChange} type="button" className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-search" aria-expanded="false">
    ...
    <div id="nav-link" className={`items-center justify-between ${navHidden ? "hidden" : ""} w-full md:flex md:w-auto md:order-1" id="navbar-search`}>
